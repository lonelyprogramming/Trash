...
public class RefreshScheduler<TKey, TValue> : IDisposable
{
        ...
        Action<TKey, TValue, bool> updateCache)
    {
        ...
        // 根据配置启动对应的刷新定时器，直接在构造函数中赋值以提高代码清晰度
    ...
    private Task ProcessSingleRefreshResult(TKey key, TValue newValue)
    {
        var cacheItem = _getCacheItem(key);
        if (cacheItem == null) return Task.CompletedTask;
...
// 主缓存类 - 专注于处理外部查询
public class ImprovedAdvancedMemoryCache<TKey, TValue> : IDisposable
{
        ...
        IDataChangeDetector<TValue> changeDetector = null)
    {
        ...
        // 关键修复：必须先初始化 _memoryCache，因为它被传递给调度器的委托所依赖。
        _memoryCache = new MemoryCache(new MemoryCacheOptions
        {
            SizeLimit = _config.MaxCacheSize
        });

        // 然后再创建刷新调度器，确保其所有依赖项都已准备就绪。
    ...
    private CacheItem<TValue> GetCacheItemForScheduler(TKey key)
    {
        ...
        var cacheKey = GetCacheKey(key);
        var result =  _memoryCache.TryGetValue(cacheKey, out CacheItem<TValue> item);
        if(!result){
            _logger?.Warning("key {cacheKey} from _memoryCache.TryGetValue get null",cacheKey);
        }
        if(item == null){
            _logger?.Warning("cacheItem is null");
        }
        return item;
    }
